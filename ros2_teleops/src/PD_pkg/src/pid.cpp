...
public:
  ...
  : Node("pd_controller_node")
  {
    ...
    controller_publisher_ = this->create_publisher<geometry_msgs::msg::Point>("controller_output", 10);
    ...
    timer_ = this->create_wall_timer(0.300ms, std::bind(&PDController::timer_callback, this));//change the ms seconds to match on the ardino code for best posting speeds
...
private:
  ...
  double sampling_time = 0.24/(pow(10.0,6.0));//in ms
